...
export class GroupLinkers {
  ...
  static async _processShop(shop, nestedData, processedUuids, parent) {
    ...
    // --- Process Inventory by calling the shared getInventory function ---
    ...
    for (const item of processedInventory) {
      // Create the itemInfo object in the structure the group sheet expects
      const itemInfo = {
        uuid: item.itemUuid,
        name: item.name,
        img: item.img,
